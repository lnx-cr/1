...
typedef struct
{
    ...
    int hex_len;                        //接受的16进制命令数组长度, 78
    long long resv_time;                //接收该指令的时间
    int correct;                        //接收数据是否完整
    //����ó��ĵ������
    ...
    float T;                            // 实际输出力矩
		float W;						// speed
    float Pos;                          // 当前电机位置
		float footForce;				// 足端气压传感器数据12bit (0-4095)

} MOTOR_recv;

#define SET_485_DE_UP() HAL_GPIO_WritePin(GPIOB, GPIO_PIN_14, GPIO_PIN_SET)
#define SET_485_DE_DOWN() HAL_GPIO_WritePin(GPIOB, GPIO_PIN_14, GPIO_PIN_RESET)

#define SET_485_RE_UP() HAL_GPIO_WritePin(GPIOB, GPIO_PIN_14, GPIO_PIN_SET)
#define SET_485_RE_DOWN() HAL_GPIO_WritePin(GPIOB, GPIO_PIN_14, GPIO_PIN_RESET)

#define SET_485_DE_UP_right() HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1, GPIO_PIN_SET)
#define SET_485_DE_DOWN_right() HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1, GPIO_PIN_RESET)

#define SET_485_RE_UP_right() HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1, GPIO_PIN_SET)
#define SET_485_RE_DOWN_right() HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1, GPIO_PIN_RESET)
...
HAL_StatusTypeDef SERVO_Send_recv_right(MOTOR_send *pData, MOTOR_recv *rData);
