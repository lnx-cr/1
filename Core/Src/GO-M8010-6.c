...
HAL_StatusTypeDef SERVO_Send_recv(MOTOR_send *pData, MOTOR_recv *rData)
{
    ...
    modify_data(pData);
		...
		SET_485_DE_UP();
		SET_485_RE_UP();
    HAL_UART_Transmit(&huart3, (uint8_t *)pData, sizeof(pData->motor_send_data), 1);
		

		SET_485_RE_DOWN();
		SET_485_DE_DOWN();
    HAL_UART_Receive(&huart3, (uint8_t *)rData, sizeof(rData->motor_recv_data), 10);
...
//    uint8_t * rp = (uint8_t *)&rData->motor_recv_data;
    if(rData->motor_recv_data.head[0] == 0xFD && rData->motor_recv_data.head[1] == 0xEE)
    {
        rData->correct = 1;
        extract_data(rData);
        return HAL_OK;
...
HAL_StatusTypeDef SERVO_Send_recv_right(MOTOR_send *pData, MOTOR_recv *rData)
{
    ...
    HAL_UART_Transmit(&huart2, (uint8_t *)pData, sizeof(pData->motor_send_data), 1);
		...
		SET_485_RE_DOWN_right();
		SET_485_DE_DOWN_right();
